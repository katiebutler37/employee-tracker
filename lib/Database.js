...
class Database {
    ...
    async getManagerNames() {
        return new Promise ((resolve, reject) => {
            db.query(`
            SELECT CONCAT(manager.first_name, ' ', manager.last_name) AS name
            FROM employees 
    ...
    async getManagerId(manager_option) {
        const params = [manager_option];
        return new Promise ((resolve, reject) => {
            db.query("SELECT id FROM employees WHERE CONCAT(first_name, ' ', last_name) = ?;", params, (err, result) => {
                if (err) throw err;
                let newResult = [];

                result.forEach((manager) => {
                    // role => { id: "5" }
                    newResult.push(manager.id); // "5"
                });
                resolve({ managerId: newResult });
            })
        })
    }
    ...
    async addEmployeeManagerQuery(managerId) {
        return new Promise ((resolve, reject) => {
            const params = [managerId];
            db.query(`
            UPDATE employees
            SET manager_id = ?
            WHERE id = MAX(id)`, params, (err, result) => {
                if (err) throw err;
                resolve();
            })
        })
